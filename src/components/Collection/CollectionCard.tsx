...
interface CollectionCardProps {
  ...
  /**
   * Requests that the parent open Quick View for this product. Quick View
   * itself now lives once at the grid level (see CollectionGrid) instead of
   * one instance per card, so this just reports "this card was activated"
   * rather than owning any modal state locally.
   */
...
function CollectionCardImpl({ product, workflow, index = 0, onQuickView }: CollectionCardProps) {
  ...
  return (
    <motion.article
      ...
      // Was `animate`, which fires the moment the card mounts — meaning all
      // 15–24 cards on a page animated simultaneously on load, regardless of
      // whether they were on-screen. `whileInView` (matching the pattern
      // already used by CollectionCarousel) defers the animation cost to
      // only the cards actually entering the viewport, which is the same
      // visual stagger for what the user sees first, at a fraction of the
      // main-thread cost on initial load.
...
// Memoized: with 15–24 cards per collection page, every pagination click or
// filter change previously re-rendered every card in the grid (including
// re-running their own modal state) even when a given card's own props
// hadn't changed. Now that Quick View lives once at the grid level, cards
// are pure w.r.t. (product, workflow, index, collectionSlug, onQuickView) —
// wrap in memo so only cards whose own props actually changed re-render.
