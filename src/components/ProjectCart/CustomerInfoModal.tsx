...
/**
 * Order (top-to-bottom) of the required fields in this form, and how each
 * maps to its input's DOM id below. Used only to drive "Review My Details"
 * in ValidationRequiredModal — validateCustomerDetails() remains the single
 * source of truth for what's actually required.
 */
const FIELD_ORDER: (keyof CustomerDetailsErrors)[] = ["name", "phone", "email"];
...
export function CustomerInfoModal({
  ...
  // Shown ONLY when required fields are missing — never for order/API
  // failures, which already have their own `submitError` UI below.
  ...
  return (
    <>
      ...
      {/* Shown only when required fields are missing — never for order/API errors */}
